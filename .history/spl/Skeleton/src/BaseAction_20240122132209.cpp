    ...
    private:
        ...
        void PrintVolunteerStatus :: act(WareHouse &wareHouse){
         // Retrieve the vector of volunteers from the warehouse
            ...
            // Flag to track whether the volunteer with the given ID is found
            bool found = false;

            // Iterate through the vector of volunteers
            for (int i = 0; i < Volunteers.size(); i++) {
               // Get the current volunteer
               Volunteer* foundVolunteer = Volunteers[i];

               // Check if the ID of the current volunteer matches the specified VolunteerId
               if (foundVolunteer->getId() == VolunteerId) {
                  // If a match is found, print the volunteer's information and set the 'found' flag to true
                  foundVolunteer->toString();
                  found = true;
                  break;
               }
            }

            // This action should result in an error if the volunteer_id doesn’t exist
            if (!found) {
               // If the volunteer with the specified ID is not found, print an error message

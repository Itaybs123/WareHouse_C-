...
, noSuchCustomer(new SoldierCustomer(-1 , "" ,-1 , -1)) , noSuchVolunteer(new LimitedCollectorVolunteer(-1,"",-1 , -1)) , noSuchOrder(new order(-1,-1,std::numeric_limits<int>::max())){
...
const vector<Order*>& WareHouse :: getVol() const{
    return vol;
}
